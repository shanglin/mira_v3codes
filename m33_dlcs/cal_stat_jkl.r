...
flcdir = '~/Work/m33_miras/m33_ofiles/dlcs/variable/'
prsdir = paste0(dir,'prsfiles/')
infdir = paste0(dir,'inffiles/')
outdir = '~/Work/m33_miras/m33_ofiles/dlcs/variable.stet.jkl.vals/'

all.fs.lc = list.files(flcdir)
lc.fields = substr(all.fs.lc,1,1)
...
## Prepare inf files and prs file to pair observations
    ...
    f.out = paste0(outdir,'m33_variable_m0',field,'.jkl')
    ts = '#      ID                     J        K       L'
    write(ts,f.out)
