...
flcdir = '~/Work/m33_miras/simulate_constant/constant_flcs/'
prsdir = paste0(dir,'prsfiles/')
infdir = paste0(dir,'inffiles/')
outdir = '~/Work/m33_miras/simulate_constant/stet.jkl.vals/'
...
for (field in fields) {
    ...
    f.out = paste0(outdir,'simu_con_m0',field,'.jkl')
    ts = '# ID      J      K      L'
    write(ts,f.out)
    fs.lc = list.files(flcdir, pattern=paste0('^con_',field,'.*.flc$'))
    nfs.lc = length(fs.lc)
